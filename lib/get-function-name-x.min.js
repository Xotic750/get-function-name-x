 ...
 */
"use strict";var getName,isFunction=_dereq_("is-function-x"),functionCtr=function(){}.constructor;if("test1"===function test1(){}.name){var createsAnonymous="anonymous"===functionCtr().name;getName=function _getName(fn){return createsAnonymous&&"anonymous"===fn.name?"":fn.name}}else{var replaceComments=_dereq_("replace-comments-x"),fToString=functionCtr.prototype.toString,normalise=_dereq_("normalize-space-x").normalizeSpace2018,reName=/^(?:async )?(?:function|class) ?(?:\* )?([\w$]+)/i,stringMatch=_dereq_("cached-constructors-x").String.prototype.match;getName=function _getName(fn){var match;try{if(match=stringMatch.call(normalise(replaceComments(fToString.call(fn)," ")),reName)){var name=match[1];return"anonymous"===name?"":name}}catch(ignore){}return""}}module.exports=function getFunctionName(fn){return isFunction(fn,!0)?getName(fn):void 0}},{"cached-constructors-x":3,"is-function-x":7,"normalize-space-x":12,"replace-comments-x":13}],2:[function(_dereq_,module,exports){/**
